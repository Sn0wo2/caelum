...
pub type FmtLayer<S = tracing_subscriber::Registry> = Box<dyn tracing_subscriber::Layer<S> + Send + Sync>;

type Layer1 = tracing_subscriber::layer::Layered<Option<FmtLayer<tracing_subscriber::Registry>>, tracing_subscriber::Registry>;
...
pub(crate) type InnerSubscriber = tracing_subscriber::layer::Layered<Option<FmtLayer<Layer4>>, Layer4>;

pub(crate) type RawReloadHandle = tracing_subscriber::reload::Handle<tracing_subscriber::EnvFilter, InnerSubscriber>;
...
{
    let formatter = build_formatter(writer, writer.color_depth.unwrap_or_else(|| {
            if writer.ansi {
        ...
        ($layer:expr $(,)?) => {{
            match &writer.target {
                ...
                WriterTarget::AsyncStdout(AsyncMode::Native) => $layer
                    .with_writer(writer::native_async_writer(writer::AsyncWriterTarget::Stdout))
                    .boxed(),
                #[cfg(feature = "custom-async")]
                ...
                WriterTarget::AsyncStderr(AsyncMode::Native) => $layer
                    .with_writer(writer::native_async_writer(writer::AsyncWriterTarget::Stderr))
                    .boxed(),
                #[cfg(feature = "file")]
                WriterTarget::File { .. } => {
    ...
    match &writer.format {
        Format::Pretty(cfg) => write_to!(
            base().pretty()
                .with_target(cfg.target).with_file(cfg.file)
                .with_line_number(cfg.line_number).with_ansi(writer.ansi)
        ),
        Format::Compact(cfg) => write_to!(
            base().with_target(cfg.target).with_file(cfg.file)
                .with_line_number(cfg.line_number).with_ansi(writer.ansi)
        ...
        Format::Json(cfg) => write_to!(
            base().json()
                ...
                .with_current_span(cfg.current_span).with_span_list(cfg.span_list)
                .flatten_event(cfg.flatten_event).with_ansi(writer.ansi)
        ),
    }
}
    ...
    S: tracing::Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>,
{
    ...
    match &writer.format {
        Format::Pretty(cfg) => base().pretty()
            .with_target(cfg.target).with_file(cfg.file)
            .with_line_number(cfg.line_number).with_ansi(false)
            .with_writer(file_writer).boxed(),
        Format::Compact(cfg) => base()
            .with_target(cfg.target).with_file(cfg.file)
            .with_line_number(cfg.line_number).with_ansi(false)
            .event_format(formatter).with_writer(file_writer).boxed(),
        Format::Json(cfg) => base().json()
            .with_target(cfg.target).with_file(cfg.file)
            .with_line_number(cfg.line_number)
            .with_current_span(cfg.current_span).with_span_list(cfg.span_list)
            .flatten_event(cfg.flatten_event).with_ansi(false)
            .with_writer(file_writer).boxed(),
    }
}
...
pub fn build_reload_filter(
    ...
    let guard = TracingGuard {
        raw, filter, style: shared_state,
        #[cfg(feature = "file")] worker_guard: None,
        #[cfg(feature = "file")] log_path: None,
    };
    (layer, guard)
}
...
pub struct SubscriberParts {
    ...
    #[cfg(feature = "file")] pub(crate) file_guard: Option<writer::LogHandle>,
    #[cfg(feature = "file")] pub(crate) log_path: Option<PathBuf>,
}
...
impl SubscriberParts {
    pub fn into_guard(self) -> TracingGuard {
        TracingGuard {
            raw: self.raw, filter: self.filter, style: self.style,
            #[cfg(feature = "file")] worker_guard: self.file_guard,
            #[cfg(feature = "file")] log_path: self.log_path,
        }
    }
    #[cfg(feature = "file")]
    pub fn file_guard(self) -> Option<writer::LogHandle> { self.file_guard }
    #[cfg(feature = "file")]
    pub fn log_path(self) -> Option<PathBuf> { self.log_path }
}
...
pub fn build_subscriber(config: Config) -> crate::Result<SubscriberParts> {
    ...
    #[cfg(feature = "file")] let mut file_guard = None;
    #[cfg(feature = "file")] let mut log_path = None;
    ...
    let filter = Filter::new(level);
    let env_filter = tracing_subscriber::EnvFilter::try_new(filter.as_directive()).unwrap_or_default();
    let (env_filter_layer, env_filter_handle) = tracing_subscriber::reload::Layer::new(env_filter);

    let subscriber = tracing_subscriber::Registry::default()
        .with(stdout_layer).with(stderr_layer)
        .with(async_stdout_layer).with(async_stderr_layer)
        .with(file_layer).with(env_filter_layer);

    let _ = tracing_log::LogTracer::init();
    tracing::subscriber::set_global_default(subscriber)?;
    Ok(SubscriberParts {
        raw: env_filter_handle, filter, style: shared_state,
        #[cfg(feature = "file")] file_guard,
        #[cfg(feature = "file")] log_path,
    })
}
...
pub struct TracingGuard {
    #[cfg(feature = "file")] #[allow(dead_code)]
...
impl std::fmt::Debug for TracingGuard {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        #[cfg(feature = "file")] d.field("log_path", &self.log_path);
...
impl TracingGuard {
    ...
    pub fn set_target_level(
        &mut self, target: impl Into<compact_str::CompactString>,
    ...
    #[cfg(feature = "file")]
    pub fn log_path(&self) -> Option<&std::path::Path> { self.log_path.as_deref() }
